...
corpuslingr::clr_ref_search_egs%>%
  ...
  DT::datatable(options = list( pageLength = nrow(clr_ref_search_egs),dom = 't'), class = 'cell-border stripe', rownames = FALSE,width="100%", escape=FALSE)%>%
...
```{r}
search3a <- c("VERB across","VERB through", "VERB out", "VERB down")
```
...
vb_prep %>%
  group_by(search) %>%
  summarize(gramx_freq = sum(txtf), gramx_type = n())
```
...
vb_prep %>%
  group_by(search)%>%
  slice(1:10)%>%
  ggplot(aes(x=reorder(lemma,txtf), y=txtf, fill = search)) + 
    geom_col(width=.6) + 
    scale_fill_stata() +
