...
description: 'A demonstration of genre-based Naive Bayes classification using R.'
author: ''
date: '2018-11-11'
...
In this short post, we outline a Naive Bayes (NB) approach to genre-based text classification.  First, we introduce & describe a corpus derived from Google News' RSS feed, which includes source and genre information.  We then train, test & evaluate the efficacy of an NB classifier applied to news genres, with some fairly nice results.  Here, we focus on the nuts/bolts of an R-based workflow, and leave discussion of theory & Bayesian assumptions for another day.
...
For demonstration purposes, I have built a fairly small corpus comprised of national news articles from Google News' RSS feed. The corpus was built using my `quicknews` package, and assembled over the course of roughly two weeks.
...
The table below summarizes the composition of our corpus in terms of genre.  So, in a little over two weeks, we have assembled a ~3K text corpus comprised of ~2 million words.  And fairly balanced as well from a genre perspective.  
