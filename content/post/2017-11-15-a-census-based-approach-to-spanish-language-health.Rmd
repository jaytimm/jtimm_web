...
spanLang %>%
  ...
  DT::datatable(options = list( pageLength = 7,dom = 't'), selection="none",class = 'cell-border stripe', rownames = FALSE,width="100%", escape=FALSE)%>%
...
```{r}
langVars <- c("B16006_001", "B16006_002", "B16006_003", "B16006_004", "B16006_005", "B16006_006", "B16006_007", "B01001I_001", "B01001_001")
```


Relevant geographies include all metropolitan statistical areas (MSAs), all US States, and the US.  
```{r}
geos <- c("us", "metropolitan statistical area/micropolitan statistical area", "state")
```
...
summaryData <- lapply(geos, function (x) {
  tidycensus::get_acs (geography= x, variables=langVars, output="wide", year=2015)}) %>%
  bind_rows() 
```
...
```{r}
geoSample <- c("1","35","10740", "29740", "22140", "42140", "26420", "12060", "38060", "16980", "19740", "31080", "12420", "27260", "33100", "46060")
```
...
summaryData %>%
  ...
  DT::datatable(options = list( pageLength = length(geoSample),dom = 't'), rownames = FALSE,width="100%", escape=FALSE)%>%
...
mp <- leaflet(divs,width="100%",height='400') %>%
      ...
      addProviderTiles ("CartoDB.Positron", options = providerTileOptions(minZoom = 4, maxZoom = 5))%>%
      addPolygons(color="gray",stroke = TRUE,weight=1,fillOpacity = 0.1) %>%
      addCircleMarkers(data=msas_w_divs, radius=3.5,fillColor=~pal(speak_span),stroke = FALSE, fillOpacity = 1,label=~paste(div,' - ',NAME))%>% 
      addLegend("bottomright", pal = pal, values = ~ msas_w_divs$speak_span, title = "Quantiles", opacity = 1)
```
...
msas_w_divs%>%
  ...
  geom_text(aes(label=NAME,col=div), size=2.5, check_overlap = TRUE,hjust = "inward")+
...
```{r}
summary(lm(SpanMono + BiLing ~ log(PerHisp) + log(TotalPop) + div, data=msas_w_divs))
```
