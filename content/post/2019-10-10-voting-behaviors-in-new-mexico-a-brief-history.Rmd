...
description: 'A brief political history of New Mexico using a host of data sources, the US Census, the New Mexico State Legislature (NMSL) & VoteView.'
---


In this post, we piece together a brief political history of New Mexico using a host of data sources, including the US Census, the New Mexico State Legislature (NMSL) & VoteView.  A bit of a show/tell post, and one that piggybacks some on a [guide](https://github.com/jaytimm/American_political_data_and_R) I have developed for working with US political data using R.  
...
```{r message=FALSE, warning=FALSE}
js <- "(/Rep/).test(value) ? '#7b9bb3' : (/Dem/).test(value) ? '#c66767' : ''"
...
So, 2018 (the 116th) was only the second time in the last thirty years that New Mexico elected an **all-Democrat delegation** to the House. And the first time an ALL FEMALE delegation!! See [this post](https://www.jtimm.net/2019/01/17/xochitl-torres-small-win-2018/) for some thoughts on how Torres Small carried New Mexico's second district in 2018.
