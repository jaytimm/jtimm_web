...
description: 'A brief political history of New Mexico using a host of data sources, including Wikipedia, the US Census, the New Mexico State Legislature (NMSL), VoteView, and the National Conference of State Legislatures.'
---


In this post, we piece together a brief political history of New Mexico using a host of data sources, including Wikipedia, the US Census, the New Mexico State Legislature (NMSL), VoteView, and the National Conference of State Legislatures.  A bit of a show/tell post, and one that piggybacks some on a [guide](https://github.com/jaytimm/American_political_data_and_R) I have developed for working with 
US political data using R.  
...
Below, we extract voting histories for all US states from Wikipedia.  Tables are mostly uniform across states (eg, [New Mexico](https://en.wikipedia.org/wiki/United_States_presidential_elections_in_New_Mexico)).  California & Pennsylvania tables, eg, are structured a bit differently, and require some individual tweaking.
...
returns <- returns[,c(1, 5)]
colnames(returns) <- c('year', 'state_winner')

returns1 <- returns %>%
  distinct() %>%
  filter(year > 1860) %>%
  left_join(pres) %>% 
  mutate(state_winner = gsub(' Howard ', ' H\\. ', state_winner),
         state_winner = gsub('Warren', 'Warren G\\.', state_winner),
         correct = ifelse(state_winner == winner, 'correct', 'incorrect'),
         state = 'Pennsylvania', 
...
returns_ca <- returns_ca[2:43,c(1:2)]
colnames(returns_ca) <- c('year', 'state_winner')

returns_ca <- returns_ca %>%
  filter(year > 1860) %>%
  mutate(correct = ifelse(grepl('^Y', state_winner), 'correct', 'incorrect'),
         state_winner = gsub('^.', '', state_winner),
         state = 'California',
         year = as.integer(year)) %>%
  select(state, year, correct)
```
...
For a quick look at the current & aggregate composition of New Mexican state leadership relative to other US states, we access data made available by the [National Conference of State Legislatures](http://www.ncsl.org/) (NCSL).  
...
```{r echo=FALSE}
table(tab$legis_control) %>% data.frame() %>% spread(Var1, Freq) %>% knitr::kable()%>% 
...
```{r echo=FALSE}
table(tab$state_control) %>% data.frame() %>% spread(Var1, Freq) %>%
  knitr::kable()%>% 
  kableExtra::kable_styling("striped", full_width = F) %>%
  kableExtra::row_spec(0, angle = -45)
```
...
Next, we investigate the party affiliation of New Mexico's governors since its statehood in 1912.  These data are made available as a PDF from the New Mexico State Legislature [website](https://www.nmlegis.gov/).
