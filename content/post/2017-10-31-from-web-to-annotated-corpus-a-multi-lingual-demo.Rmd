...
link-citations: yes
banner: banners/build_corpus.jpg
---
...
```{r message=FALSE, warning=FALSE}
us_nation_annotated <- cleanNLP::cnlp_annotate (us_nation_corpus$text, as_strings = TRUE) %>%
  .$token %>%
  filter(lemma !='ROOT') %>%
...
```{r fig.height=5.5}
corpuslingr::clr_desc_corpus(us_nation_annotated,doc='id',sent='sid', tok='word', upos='upos')$text %>%
  inner_join(us_nation_corpus, by = c('id'='doc_id'))%>%
  ggplot(aes(x=textLength, y=textType,color=source, size=textSent))+
      geom_point()
```
...
```{r}
es_mx_world_corp <- quicknews::qnews_get_meta (language="es",country="mx",type="topic",search="world")%>% 
  quicknews::qnews_scrape_web ()
```
